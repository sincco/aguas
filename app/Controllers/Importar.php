	...
	public function contratos() {
		...
		if (!$conn) {
			...
			while ($padron = oci_fetch_assoc($stid)) {
				...
				//var_dump($padron);
				$query = "INSERT INTO contratos (contrato, altaContrato, propietario, usuario, suministro, numTomas, giro, utilizacion, tarifa, servicios, nivelTarifario, asociacion, cveCatastral, municipio, colonia, via, calle, numOficial, longitud, latitud, serieMedidor, telemetriaMedidor, interior, interior2, marca_act, num_apa, aparato, tip_medicion, f_inst, anos_disp_med, mayores_5_anos, f_retiro_disp_med, cobro, mod_med_ins, tipo_material, caudal_max, num_dig_medidor, f_fabricacion, diametro_toma, lectura_inicial, region, estrato, bandera, USUARIO_S, ESTADO_ORDEN, CANTIDAD_MEDIDORES, NUM_OS, CON_PAGO, DIAMETRO_CONEXION, TEL_PROP, CEL_PROP, TEL_TP, CEL_TP, COMENT_OS, INTERIOR_2) VALUES ('" . $padron["NIS"] . "','" . $padron["ALTACONTRATO"] . "','" . str_replace("'", '', $padron["PROPIETARIO"]) . "','" . str_replace("'", '', $padron["USUARIO"]) . "','" . str_replace("'", '', $padron["SUMINISTRO"]) . "','" . str_replace("'", '', $padron["NUMTOMAS"]) . "','" . str_replace("'", '', $padron["GIRO"]) . "','" . str_replace("'", '', $padron["UTILIZACION"]) . "','" . str_replace("'", '', $padron["TARIFA"]) . "','" . str_replace("'", '', $padron["SERVICIOS"]) . "','" . str_replace("'", '', $padron["NIVELTARIFARIO"]) . "','" . str_replace("'", '', $padron["ASOCIACION"]) . "','" . str_replace("'", '', $padron["CVECATASTRAL"]) . "','" . str_replace("'", '', $padron["MUNICIPIO"]) . "','" . str_replace("'", '', $padron["COLONIA"]) . "','" . str_replace("'", '', $padron["VIA"]) . "','" . str_replace("'", '', $padron["CALLE"]) . "','" . str_replace("'", '', $padron["NUMOFICIAL"]) . "','','','','','" . str_replace("'", '', $padron["INTERIOR_1"]) . "','" . str_replace("'", '', $padron["INTERIOR_2"]) . "','" . str_replace("'", '', $padron["MARCA_ACT"]) . "','" . str_replace("'", '', $padron["NUM_APA"]) . "','" . str_replace("'", '', $padron["APARATO"]) . "','" . str_replace("'", '', $padron["TIP_MEDICION"]) . "','" . str_replace("'", '', $padron["F_INST"]) . "','" . str_replace("'", '', $padron["ANOS_DISP_MED"]) . "','" . str_replace("'", '', $padron["MAYORES_5_ANOS"]) . "','" . str_replace("'", '', $padron["F_RETIRO_DISP_MED"]) . "','','','','',0,NULL,'',0,'" . str_replace("'", '', $padron["REGION"]) . "','" . str_replace("'", '', $padron["ESTRATO"]) . "','" . str_replace("'", '', $padron["BANDERA"]) . "','" . $padron['USUARIO_S'] . "','" . $padron['ESTADO_ORDEN'] . "','" . $padron['CANTIDAD_MEDIDORES'] . "','" . $padron['NUM_OS'] . "','" . $padron['CON_PAGO'] . "','" . $padron['DIAMETRO_CONEXION'] . "','" . $padron['TEL_PROP'] . "','" . $padron['CEL_PROP'] . "','" . $padron['TEL_TP'] . "','" . $padron['CEL_TP'] . "','" . str_replace('"', '', str_replace("'", '', $padron['COMENT_OS'])) . "','" . str_replace('"', '', str_replace("'", '', $padron['INTERIOR_2'])) . "') ON DUPLICATE KEY UPDATE interior2='" . str_replace("'", '', $padron["INTERIOR_2"]) . "',region='" . str_replace("'", '', $padron["REGION"]) . "',estrato='" . str_replace("'", '', $padron["ESTRATO"]) . "',bandera=" . str_replace("'", '', $padron["BANDERA"]) . ", USUARIO_S='" . $padron['USUARIO_S'] . "', ESTADO_ORDEN='" . $padron['ESTADO_ORDEN'] . "',CANTIDAD_MEDIDORES='" . $padron['CANTIDAD_MEDIDORES'] . "',NUM_OS='" . $padron['NUM_OS'] . "',CON_PAGO='" . $padron['CON_PAGO'] . "',DIAMETRO_CONEXION='" . $padron['DIAMETRO_CONEXION'] . "',TEL_PROP='" . $padron['TEL_PROP'] . "',CEL_PROP='" . $padron['CEL_PROP'] . "',TEL_TP='" . $padron['TEL_TP'] . "',CEL_TP='" . $padron['CEL_TP'] . "',COMENT_OS='" . str_replace('"', '', str_replace("'", '', $padron['COMENT_OS'])) . "', DUPLICADOR='" . str_replace('"', '', str_replace("'", '', $padron['INTERIOR_2'])) . "';";
