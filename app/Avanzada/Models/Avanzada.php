...
class AvanzadaModel extends Sincco\Sfphp\Abstracts\Model {
	...
	public function getTable($data, $cuadrilla = 0) {
		...
		$query = "SELECT asg.contrato, con.altaContrato, con.propietario, con.utilizacion, con.tarifa,con.interior, CONCAT(con.via, ' ', con.calle, ' ', con.numOficial) direccion, con.colonia, con.municipio, asg.fechaAsignacion, est.descripcion estatus, ven.nombre, vis.observaciones, ven.nombre,est.descripcion,vis.observaciones
			FROM avanzadaContratosAsignados asg
			INNER JOIN contratos con USING (contrato)
			INNER JOIN avanzadaEstatus est USING (estatusId)
			LEFT JOIN avanzadaVisita vis USING (contrato)
